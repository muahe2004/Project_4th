...
const getFileNameFromDisposition = (contentDisposition?: string) => {
  if (!contentDisposition) {
    return "teaching_schedules_template.xlsx";
  }
...
const exportExampleFile = async (): Promise<void> => {
  const response = await apiClient.get(`${URL_API_EXPORT}/teaching-schedules-template`, {
    responseType: "blob",
  });
...
export const useExportExampleFile = ({ config }: UseExportExampleFileOptions = {}) =>
  useMutation({
