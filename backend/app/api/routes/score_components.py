...
@router.get("/{id}", response_model=ScoreComponentPublic)
def get_score_component_by_id(
    session: SessionDep, id: uuid.UUID, request: Request
) -> ScoreComponentPublic:
    return ScoreComponentServices.get_by_id(
        session=session, score_component_id=id, request=request
    )
...
def create_score_component(
    request: Request, session: SessionDep, data: ScoreComponentCreate
) -> ScoreComponentPublic:
...
def update_score_component(
    session: SessionDep, id: uuid.UUID, data: ScoreComponentUpdate
) -> ScoreComponentPublic:
    return ScoreComponentServices.update(
        session=session, score_component_id=id, score_component_data=data
    )
