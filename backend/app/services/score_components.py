...
class ScoreComponentServices:
    ...
    def get_by_id(
        *,
        ...
        if not score_component:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Score component does not exist"
            )
    ...
    def create(
        *,
        session: Session,
        score_component: ScoreComponentCreate
    ) -> ScoreComponentPublic:
        ...
        if existing:
            raise HTTPException(
                ...
                detail=f"Score conponent already exists.",
            )
    ...
    def update(
        *,
        ...
        if not score_component:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Score component not found"
            )
    ...
    def delete(
        *,
        ...
        check_related_entities = select(Scores).where(Scores.score_component_id == score_component.id)
        ...
        if score_component.status == StatusEnum.ACTIVE:
            ...
            session.commit()
            return ScoreComponentDeleteResponse(id = str(score_component.id), message = "Score component set to inactive")
    
        session.delete(score_component)
        session.commit()

        return ScoreComponentDeleteResponse(id = str(score_component.id), message = "Score component deleted successfully")