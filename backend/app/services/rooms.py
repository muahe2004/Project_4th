...
class RoomServices:
    ...
    def delete(
        *,
        ...
        check_related_entities = select(LearningSchedules).where(LearningSchedules.room_id == room.id)
        ...
        check_related_entities_second = select(ExaminationSchedules).where(ExaminationSchedules.room_id == room.id)
